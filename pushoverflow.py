...
def sendToPushover(pushover_config, title, message, url=None):
...
def sendQuestionsToPushover(pushover_config, exchange, questions):
    ...
    if len(questions)==1:
        message = "New question posted: " + questions[0].get("title")
        url = questions[0].get("link")
    else:
        message = str(len(questions)) + " new questions posted."
        url = "http://" + exchange.name + ".stackexchange.com/questions?sort=newest"
    sendToPushover(pushover_config, title, message, url)
...
def checkExchange(exchange, from_date):
    ...
    questions = filterQuestions(questions, tags, excluded)
    return questions
...
def main(argv):
    ...
    try:
        ...
        pushover_app_key = config.get("Pushover","appkey")
        pushover_user_key = config.get("Pushover","userkey")
    ...
    for section in config.sections():
        ...
        questions = checkExchange(exchange,from_date)
        if len(questions) > 0:
            sendQuestionsToPushover(config["Pushover"], exchange, questions)
